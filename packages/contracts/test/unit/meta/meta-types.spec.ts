...
describe('generated meta objects', () => {
  ...
  it.each(allMetas)('$name has at least one field', ({ meta }) => {
    expect(Object.keys(meta).length).toBeGreaterThan(0);
  });

  it.each(allMetas)('$name has valid type discriminants on all fields', ({ meta }) => {
  ...
  it.each(allMetas)('$name has nullable as a boolean on all fields', ({ meta }) => {
    for (const [key, field] of Object.entries(meta)) {
      expect(typeof field.nullable, `field "${key}" has non-boolean nullable`).toBe('boolean');
    }
  });

  it.each(allMetas)('$name enum fields have non-empty string values arrays', ({ meta }) => {
    for (const [key, field] of Object.entries(meta)) {
      if (field.type === 'enum') {
  ...
  it.each(allMetas)('$name non-enum fields do not have a values property', ({ meta }) => {
    for (const [key, field] of Object.entries(meta)) {
      if (field.type !== 'enum') {
