...
import { InsurUpClientErrorType, InsurUpGraphQLErrorCode } from '../../src/core/result';
  ...
  describe('Error Handling', () => {
    it('should handle GraphQL errors in response with graphql-error kind', async () => {
      ...
      if (result.kind === 'graphql-error') {
        ...
        expect(result.errors[0]?.extensions?.code).toBe(InsurUpGraphQLErrorCode.ValidationError);
      }
    });
    ...
    it('should handle multiple GraphQL errors', async () => {
      ...
      if (result.kind === 'graphql-error') {
        ...
        expect(result.errors[0]?.extensions?.code).toBe(InsurUpGraphQLErrorCode.BadRequest);
        expect(result.errors[1]?.message).toBe('Error 2');
        expect(result.errors[1]?.extensions?.code).toBe(InsurUpGraphQLErrorCode.Forbidden);
      }
    });
    ...
    it('should handle GraphQL errors with full extensions', async () => {
      ...
      expect(result.kind).toBe('graphql-error');
      if (result.kind === 'graphql-error') {
        expect(result.errors[0]?.extensions?.code).toBe(InsurUpGraphQLErrorCode.NotFound);
        expect(result.errors[0]?.extensions?.traceId).toBe('abc-123-def');
        expect(result.errors[0]?.extensions?.codes).toEqual([
          'CUSTOMER_NOT_FOUND',
          'ENTITY_MISSING',
        ]);
        expect(result.errors[0]?.extensions?.template).toBe('Customer with ID {id} was not found');
