...
import type { Connection, QueryCustomerModel } from '@insurup/contracts';
import { DateTime, DateOnly, SortEnumType } from '@insurup/contracts';
...
describe('InsurUpCustomerClient.getCustomers', () => {
  ...
  const createMockCustomer = (overrides?: Partial<QueryCustomerModel>): QueryCustomerModel => ({
    id: 'customer-123',
    name: 'John Doe',
  ...
  describe('Basic Queries', () => {
    it('should fetch customers with all fields by default', async () => {
      const mockCustomers = [
        createMockCustomer(),
        createMockCustomer({ id: 'customer-456', name: 'Jane Doe' }),
      ];
      ...
      mockFetch.mockResolvedValueOnce({
        ...
        text: () => Promise.resolve(JSON.stringify({ data: { customersNew: mockConnection } })),
      });
  ...
  describe('Pagination', () => {
    it('should support forward pagination with first/after', async () => {
      const mockConnection: Connection<QueryCustomerModel> = {
        pageInfo: {
          hasNextPage: true,
          hasPreviousPage: false,
      ...
      await customerClient.getCustomers({
        ...
        filter: {
          and: [
            { type: { eq: CustomerType.Individual } },
            {
              or: [{ cityText: { contains: 'Istanbul' } }, { cityText: { contains: 'Ankara' } }],
            },
          ],
        },
      });
