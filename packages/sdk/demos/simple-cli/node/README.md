...
- **Credential Storage**: Secure token persistence using keytar (OS keychain)
...
│   ├── credential-store.ts   # Token persistence (keytar)
│   └── callback-server.ts    # OAuth callback handler (node:http)
└── actions/
...
This demo uses [keytar](https://github.com/atom/node-keytar) for secure credential storage:

- **macOS**: Keychain
- **Windows**: Credential Vault
- **Linux**: Secret Service API (libsecret)

Tokens are stored securely in your OS keychain and persist across CLI sessions.
