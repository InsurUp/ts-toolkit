...
export const action: Action = {
  ...
  async execute() {
    ...
    lines.push(
      `  Status: ${authStatus.isAuthenticated ? color.green('Authenticated') : color.red('Not authenticated')}`
    );
    ...
    if (authStatus.isAuthenticated) {
      ...
      lines.push(
        `  Auto-refresh: ${authStatus.hasRefreshToken ? color.green('enabled') : color.dim('disabled')}`
      );
    }
