<script lang="ts">
  ...
  let showPinMenu = $state<string | null>(null);
  let pinMenuPosition = $state<{ top: number; left: number } | null>(null);

  // ============================================================================
  // Controlled table state via $state
  // These are the source of truth, bound to tableOptions.state reactively.
  // ============================================================================
  ...
  // Create customer table with controlled state via tableOptions.state
  // The getter function re-evaluates when $state variables change,
  // and $effect.pre in createTableCore syncs them to the table.
  const ct = createCustomerTable(() => ({
    ...
    tableOptions: {
      ...
      enablePinning: true,
      // Controlled state — $state variables are the source of truth
      state: {
        sorting,
        columnVisibility,
        columnOrder,
      ...
      // Sync table-initiated changes back to $state.
      // Called by the adapter after it processes the change (refetch, etc.).
      onStateChange: () => {
        const s = ct.table.getState();
        sorting = s.sorting;
        columnVisibility = s.columnVisibility;
        columnOrder = s.columnOrder;
        columnPinning = s.columnPinning;
        columnSizing = s.columnSizing;
        columnSizingInfo = s.columnSizingInfo;
      },
    },
  }));
  ...
  function toggleColumn(columnKey: string): void {
    columnVisibility = {
      ...columnVisibility,
      [columnKey]: columnVisibility[columnKey] === false,
    };
  }
  ...
  // Toggle sorting via $state mutation
  function toggleSort(columnId: string): void {
    const descFirst = ct.table.getColumn(columnId)?.columnDef.sortDescFirst ?? false;
    const current = sorting.find((s) => s.id === columnId);

    if (!current) {
      sorting = [{ id: columnId, desc: descFirst }];
    } else if (current.desc === descFirst) {
      sorting = [{ id: columnId, desc: !descFirst }];
    } else {
      sorting = [];
    }
  ...
  // Read sorting direction from $state
  ...
  // Track resizing column from controlled $state
  let resizingColumnId = $derived(columnSizingInfo.isResizingColumn);
  ...
  function handleDndFinalize(e: CustomEvent<DndEvent<DndHeaderItem>>): void {
    ...
    dndItems = finalItems;
    columnOrder = finalItems.map((item) => item.id);
  }
  ...
  // Column pinning via $state mutation
  ...
  // Read pin state from controlled $state
  ...
  function openPinMenu(e: MouseEvent, columnId: string): void {
    ...
    if (showPinMenu === columnId) {
      ...
      pinMenuPosition = {
        top: rect.bottom + 4,
        left: Math.min(rect.right - 128, window.innerWidth - 140),
      };
...
<div class="space-y-6">
  <div class="flex items-center justify-between">
    <div>
      <h1 class="text-3xl font-bold tracking-tight">Customers (Controlled)</h1>
      <p class="text-muted-foreground">
        Table state managed via <code class="text-xs bg-muted px-1 py-0.5 rounded">$state</code> bound to <code class="text-xs bg-muted px-1 py-0.5 rounded">tableOptions.state</code>.
      </p>
    </div>
    <button
  ...
  <div class="flex items-center gap-4">
    ...
    <!-- Column visibility dropdown — reads from controlled $state -->
    <div class="relative">
      <button
        class="inline-flex items-center gap-2 h-9 px-4 rounded-md border bg-background hover:bg-accent"
            ...
            {#if header}
              ...
              >
                ...
                <button
                  ...
                  ondblclick={() => {
                    const { [header.column.id]: _, ...rest } = columnSizing;
                    columnSizing = rest;
                  }}
...
<!-- Pin menu dropdown (fixed positioning) -->
