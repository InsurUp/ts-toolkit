<script lang="ts">
  ...
  // Create infinite customer table instance with resizing enabled
  ...
  // Note: createVirtualizer returns a store in svelte-virtual
  let virtualizer = $derived(createVirtualizer<HTMLDivElement, HTMLTableRowElement>({
  ...
  function loadMore(): void {
    if (ct.table.getCanNextPage() && !ct.isFetching) {
      ct.table.nextPage();
    }
  }
  ...
  // Get current visible headers for DnD
  ...
  let hasMoreData = $derived(ct.table.getCanNextPage());
  let totalLoaded = $derived(ct.rows.length);
</script>
  ...
  <div class="flex items-center justify-between">
    <div>
      ...
      <p class="text-muted-foreground">
        Customer table with infinite scroll, column pinning, drag-and-drop ordering, and resizing.
      </p>
    </div>
    <button
      class="inline-flex items-center gap-2 h-9 px-4 rounded-md border bg-background hover:bg-accent disabled:opacity-50"
      disabled={ct.isFetching}
      onclick={handleRefresh}
    >
      <RefreshCw class="h-4 w-4 {ct.isFetching ? 'animate-spin' : ''}" />
      Refresh
    </button>
  </div>
  ...
  >
    <div style="position: relative; height: {$virtualizer.getTotalSize()}px;">
      <table class="w-full caption-bottom text-sm" style="table-layout: fixed;">
        <thead class="[&_tr]:border-b bg-muted/50 sticky top-0 z-10">
          <tr
            use:dndzone={{
              items: dndItems,
              flipDurationMs: 200,
        ...
        <tbody class="[&_tr:last-child]:border-0">
          {#if ct.isLoading && ct.rows.length === 0}
            {#each {length: 5} as _}
          ...
          {:else}
            ...
            {#each $virtualizer.getVirtualItems() as virtualRow, idx (virtualRow.index)}
              {@const row = ct.rows[virtualRow.index]}
              {@const rowData = row as Record<string, unknown>}
              <tr
                data-index={virtualRow.index}
                class="flex w-full border-b hover:bg-muted/50"
                style="height: {virtualRow.size}px; transform: translateY({virtualRow.start - idx * virtualRow.size}px);"
              >
                {#each dndItems as item (getDndItemKey(item))}
                  {@const header = getHeader(item.id)}
                  {@const value = rowData[item.id]}
                  {@const isShadowPlaceholder = !header && item.id.includes('dnd-shadow-placeholder')}
                  {#if header}
                    <td 
                      class="p-2 align-middle overflow-hidden truncate flex items-center {getPinnedClass(header)}" 
                      style="width: {getPercentWidth(header)}%; {getStickyStyle(header)}"
                    >
                      {#if item.id === "type"}
    ...
    {#if ct.isFetching && ct.hasData}
      <div class="flex items-center justify-center py-4 sticky bottom-0 bg-background/80 backdrop-blur-sm">
    ...
    {#if !hasMoreData && ct.hasData && !ct.isFetching}
      <div class="flex items-center justify-center py-4">
        <span class="text-sm text-muted-foreground">
          All {ct.rowCount} customers loaded
        </span>
      </div>
    {/if}
  </div>
