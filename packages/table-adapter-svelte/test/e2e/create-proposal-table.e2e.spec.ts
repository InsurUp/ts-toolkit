/**
 * @fileoverview E2E: createProposalTable factory against the real InsurUp API.
 *
...
import { createProposalTable } from '../../src/lib/create-proposal-table.svelte.js';
...
describeE2E('createProposalTable [e2e]', () => {
  it('fetch updates the reactive rows signal', async () => {
    ...
    const result = createProposalTable(() => ({
  ...
  it('setFilter updates the rows signal with constrained data', async () => {
    ...
    try {
      result.adapter.setFilter({ state: { eq: ProposalState.Active } });
